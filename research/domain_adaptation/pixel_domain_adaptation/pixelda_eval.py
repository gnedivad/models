...
flags.DEFINE_string('source_dataset', 'lung',
                    'The name of the source dataset.')

flags.DEFINE_string('target_dataset', 'lung_m',
                    'The name of the target dataset.')
  ...
  for checkpoint_path in slim.evaluation.checkpoints_iterator(
    ...
    with tf.Graph().as_default():
      ...
      target_dataset = dataset_factory.get_dataset(
          ...
          split_name=FLAGS.target_split_name,
          dataset_dir=FLAGS.dataset_dir)
      target_real_images, target_virtual_images, target_labels = (
        dataset_factory.provide_batch(
            FLAGS.target_dataset,
            FLAGS.target_split_name,
            FLAGS.dataset_dir,
            FLAGS.num_readers,
            hparams.batch_size,
            FLAGS.num_preprocessing_threads)
      ...
      pixelda_utils.summarize_model(end_points)
      pixelda_utils.summarize_transferred_grid(
          end_points['transferred_real_images'], source_real_images, name='Transferred')
      if 'source_real_images_recon' in end_points:
        pixelda_utils.summarize_transferred_grid(
            end_points['source_real_images_recon'],
            source_real_images,
            name='Source Reconstruction')
      pixelda_utils.summarize_images(target_real_images, 'Target')
...
def create_metrics(end_points, source_labels, target_labels, hparams):
  ...
  if 'source_task_logits' in end_points:
    ...
    logits_op = end_points['source_task_logits']
    labels_op = source_labels
    logits_xyz_op = tf.slice(logits_op, [0, 0], [-1, 3])
    logits_aer_op = tf.slice(logits_op, [0, 3], [-1, -1])
    labels_xyz_op = tf.slice(labels_op, [0, 0], [-1, 3])
    labels_aer_op = tf.slice(labels_op, [0, 3], [-1, -1])
    xyz_losses = tf.sqrt(tf.reduce_sum(tf.square(logits_xyz_op - labels_xyz_op), axis=1))
    aer_losses = tf.sqrt(tf.reduce_sum(tf.square(logits_aer_op - labels_aer_op), axis=1))

    names_to_values[metric_name], names_to_updates[
        metric_name] = tf.contrib.metrics.streaming_mean(xyz_losses)
  if 'transferred_task_logits' in end_points:
    metric_name = 'eval/Task_Accuracy-Transferred'

    logits_op = end_points['transferred_task_logits']
    labels_op = source_labels
    logits_xyz_op = tf.slice(logits_op, [0, 0], [-1, 3])
    logits_aer_op = tf.slice(logits_op, [0, 3], [-1, -1])
    labels_xyz_op = tf.slice(labels_op, [0, 0], [-1, 3])
  ...
  if 'target_task_logits' in end_points:
    ...
    logits_op = end_points['target_task_logits']
    labels_op = target_labels
    logits_xyz_op = tf.slice(logits_op, [0, 0], [-1, 3])
    logits_aer_op = tf.slice(logits_op, [0, 3], [-1, -1])
    labels_xyz_op = tf.slice(labels_op, [0, 0], [-1, 3])
    labels_aer_op = tf.slice(labels_op, [0, 3], [-1, -1])
