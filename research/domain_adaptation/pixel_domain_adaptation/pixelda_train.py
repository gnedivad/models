...
#           return loss
#         loss, np_global_step, _source_images = session.run(
#             [discriminator_train_op, global_step, source_images])
#         if (np_global_step + 1) % FLAGS.print_loss_steps == 0:
...
flags.DEFINE_string('source_dataset', 'lung', 'The name of the source dataset.'
                    ' If hparams="arch=dcgan", this flag is ignored.')
  ...
  with tf.Graph().as_default():
    with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
      ...
      generator_loss = pixelda_losses.g_step_loss(
          ...
          # num_classes=num_target_classes)
      discriminator_loss = pixelda_losses.d_step_loss(
          end_points, source_labels, hparams)  # num_target_classes, hparams)
      ...
      if 'source_real_images_recon' in end_points:
        pixelda_utils.summarize_transferred_grid(
            ...
            source_real_images,
            name='Source Reconstruction')
      pixelda_utils.summaries_color_distributions(end_points['transferred_real_images'],
                                                  'Transferred')
      pixelda_utils.summaries_color_distributions(target_real_images, 'Target')

      if source_real_images is not None:
        pixelda_utils.summarize_transferred(source_real_images,
                                            end_points['transferred_real_images'])
        pixelda_utils.summaries_color_distributions(source_real_images, 'Source')
        pixelda_utils.summaries_color_distributions(
            tf.abs(source_real_images - end_points['transferred_real_images']),
            'Abs(Source_minus_Transferred)')
