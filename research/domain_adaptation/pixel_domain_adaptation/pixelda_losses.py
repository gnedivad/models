...
def _add_task_specific_losses(end_points, source_labels, hparams,
                              add_summaries=False):
  """Adds losses related to the task-classifier.
  ...
  if 'source_task_logits' in end_points:
    ...
    xyz_loss = tf.reduce_mean(
      tf.sqrt(tf.reduce_sum(tf.square(logits_xyz_op - labels_xyz_op), axis=1)),
      name="xyz_loss",
    )

    aer_loss = tf.reduce_mean(
      tf.sqrt(tf.reduce_sum(tf.square(logits_aer_op - labels_aer_op), axis=1)),
      name="aer_loss",
    )

    loss = xyz_loss + aer_loss
    # loss = tf.losses.softmax_cross_entropy(
    #     onehot_labels=one_hot_labels,
    #     logits=end_points['source_task_logits'],
    #     weights=hparams.source_task_loss_weight)
...
def d_step_loss(end_points, source_labels, hparams):  # num_classes, hparams):
  """Configures the losses during the D-Step.
  ...
  if source_labels is not None:
    task_classifier_loss = _add_task_specific_losses(
        end_points, source_labels, hparams, add_summaries=True)  # num_classes, hparams, add_summaries=True)
