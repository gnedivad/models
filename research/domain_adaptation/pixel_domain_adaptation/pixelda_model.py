...
def create_model(hparams,
                 target_real_images,
                 target_virtual_images,
                 source_real_images=None,
                 source_virtual_images=None,
                 source_labels=None,
                 is_training=False,
                 noise=None):
                 # num_classes=None):
  ...
  Arguments:
    hparams: HParam object specifying model params
    target_real_images: A `Tensor` of size [batch_size, height, width, channels].
      It is assumed that the images are [-1, 1] normalized.
    target_virtual_images: A `Tensor` of size [batch_size, height, width, channels].
      It is assumed that the images are [-1, 1] normalized.
    source_real_images: A `Tensor` of size [batch_size, height, width, channels].
      It is assumed that the images are [-1, 1] normalized.
    source_virtual_images: A `Tensor` of size [batch_size, height, width, channels].
      It is assumed that the images are [-1, 1] normalized.
    source_labels: A `Tensor` of size [batch_size, 6] of labels.
    is_training: whether model is currently training
    noise: If None, model generates its own noise. Otherwise use provided.
    # num_classes: Number of classes for classification
  ...
  if (target_real_images.dtype != tf.float32
      or target_virtual_images.dtype != tf.float32):
    raise ValueError('target_real_images and target_virtual_images must be '
                     'tf.float32 and [-1, 1] normalized.')
  if ((source_real_images is not None and source_real_images.dtype != tf.float32)
      or (source_virtual_images is not None and source_virtual_images.dtype != tf.float32)):
    raise ValueError('source_real_images and source_virtual_images must be '
                     'tf.float32 and [-1, 1] normalized.')
  ...
  with slim.arg_scope(
    ...
    with slim.arg_scope([slim.conv2d], padding='SAME'):
      if hparams.arch == 'dcgan':
        end_points = dcgan(
            target_real_images, latent_vars, hparams, scope='generator')
      elif hparams.arch == 'resnet':
        end_points = resnet_generator(
            source_real_images,
            target_real_images.shape.as_list()[1:4],
            hparams=hparams,
            latent_vars=latent_vars)
      elif hparams.arch == 'residual_interpretation':
        end_points = residual_interpretation_generator(
            source_real_images, is_training=is_training, hparams=hparams)
      elif hparams.arch == 'simple':
        end_points = simple_generator(
            source_real_images,
            target_real_images,
        ...
        if hparams.generator_steps:
          raise ValueError('Must set generator_steps=0 for identity arch. Is %s'
                           % hparams.generator_steps)
        transferred_real_images = source_real_images
        source_channels = source_real_images.shape.as_list()[-1]
        target_channels = target_real_images.shape.as_list()[-1]
        if source_channels == 1 and target_channels == 3:
      ...
      if hparams.arch in [
        ...
        # Add a discriminator for these architectures
        end_points['transferred_domain_logits'] = predict_domain(
            end_points['transferred_real_images'],
            hparams,
            is_training=is_training,
            reuse=False)
        end_points['target_domain_logits'] = predict_domain(
      ...
      if hparams.task_tower != 'none' and hparams.arch in [
        ...
        with tf.variable_scope('discriminator'):
          with tf.variable_scope('task_tower'):
            ...
            #         private_scope='transferred_task_classifier',
            #         reuse_shared=True)
            end_points['source_task_logits'] = pixelda_task_towers.add_task_specific_model(
                    source_real_images,
                    source_virtual_images,
                    hparams,
                    # num_classes=num_classes,
                    is_training=is_training,
                    reuse_private=False,
                    private_scope='source_task_classifier',
                    reuse_shared=False)
            end_points['transferred_task_logits'] = (
                    pixelda_task_towers.add_task_specific_model(
                        end_points['transferred_real_images'],
                        source_virtual_images,
                        hparams,
                        ...
                        private_scope='transferred_task_classifier',
                        reuse_shared=True))
            end_points['target_task_logits'] = pixelda_task_towers.add_task_specific_model(
                    target_real_images,
                    target_virtual_images,
                    hparams,
                    # num_classes=num_classes,
  ...
  with tf.variable_scope(scope, 'resnet_style_transfer', [images]):
    with slim.arg_scope(
      ...
      net = slim.conv2d(
          ...
          activation_fn=tf.nn.tanh,
          scope='conv_out')
      end_points['transferred_real_images'] = net
    return net, end_points
