...
from domain_adaptation.datasets import lung, lung_m
...
def get_dataset(dataset_name,
  ...
  Raises:
    ValueError: if `dataset_name` isn't recognized.
  """
  dataset_name_to_module = {'lung': lung, 'lung_m': lung_m}
  if dataset_name not in dataset_name_to_module:
    raise ValueError('Name of dataset unknown %s.' % dataset_name)
...
def provide_batch(dataset_name, split_name, dataset_dir, num_readers,
  ...
  provider = slim.dataset_data_provider.DatasetDataProvider(
    ...
    common_queue_capacity=20 * batch_size,
    common_queue_min=10 * batch_size)
  [real_image, virtual_image, label] = provider.get(['real_image', 'virtual_image', 'label'])

  # Convert images to float32 [-1.0, 1.0]
  real_image = tf.image.convert_image_dtype(real_image, tf.float32)  # [0.0, 1.0]
  real_image -= 0.5
  real_image *= 2

  virtual_image = tf.image.convert_image_dtype(virtual_image, tf.float32)  # [0.0, 1.0]
  virtual_image -= 0.5
  virtual_image *= 2

  # Load the data.
  real_images, virtual_images, labels = tf.train.batch(
      [real_image, virtual_image, label],
      batch_size=batch_size,
      num_threads=num_preprocessing_threads,
      capacity=5 * batch_size)

  return real_images, virtual_images, labels
